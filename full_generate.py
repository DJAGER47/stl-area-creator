...
def generate(obl_name: str, path_save: str, step_m: int, gpkd) -> None:
    ...
    print(f'utm  : {utm_geo_original.crs}')
    ...
    for poly in utm_contour_raw:
        if len(poly) > 2:
            ...
            full_point_x = int((maxx - minx) / step)
            if full_point_x > 0:
    ...
    for i, polygon in enumerate(utm_contour_geometry):
        ...
        full_point_x = int((maxx - minx) / step)
        for j, x in enumerate(range(int(minx), int(maxx) + 1, step)):
    ...
    print(f"inside filtred  time: {print_time():.2f}s")
    print(f'Mesh done! point {len(utm_mesh[0])}')
