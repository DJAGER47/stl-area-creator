...
if __name__ == "__main__":
    ...
    #     generate(obl_name, path, step, oblast, water)
    # else:
    #     for i in range(0, len(gpkd["region"])):
    #         oblast = gpkd[gpkd["region"] == gpkd["region"][i]]
    #         generate(gpkd["region"][i], path, step, oblast, water)

    # oblast = None
    # with open("circle.geojson", 'r') as f:
    #         circle = geojson.load(f)
    #         oblast = circle['features'][0]
    gpkd = gpd.read_file("circle.geojson")
    generate("circle", path, 100, gpkd, water)
